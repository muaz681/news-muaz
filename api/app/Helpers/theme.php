...
if(!function_exists('getSiteLogo')){
    function getSiteLogo(){
        ...
        $logo = $site_logo != null ? asset('storage').'/'. $site_logo : '/assets/images/no-logo.png' ;
        return $logo;
    }
}
...
if(!function_exists('getHeaderImage')){
    function getHeaderImage(){
        $bg_type = Setting::where('key','bg_type')->value('value');
        $bg_image = Setting::where('key','bg_image')->value('value');
        $bg_color = Setting::where('key','bg_color')->value('value');
        $header_image = $bg_type == 'Image' && $bg_image != null ? 'url('.asset('storage').'/'. $bg_image .')' : $bg_color ;
        return $header_image;
    }
}

if(!function_exists('getFooterImage')){
    function getFooterImage(){
        $bg_type = Setting::where('key','bg_type')->value('value');
        $bg_image = Setting::where('key','bg_image')->value('value');
        $bg_color = Setting::where('key','bg_color')->value('value');
        $footer_image = $bg_type == 'Image' && $bg_image != null ? 'url('.asset('storage').'/'. $bg_image .')' : $bg_color ;
        return $footer_image;
    }
}

if(!function_exists('getHeaderMenuImage')){
    function getHeaderMenuImage(){
        $site_logo = Setting::where('key','bg_image')->pluck('value');
        $logo = asset('storage').'/'. $site_logo[0];
        return $logo;
    }
}
...
if(!function_exists('getSiteName')){
    function getSiteName(){
        $title = Setting::where('key','website_name')->value('value');
        return $title;
    }
}
...
if(!function_exists('footerSetting')){
    function footerSetting(){
        $footer_setting = Setting::where('key','footer_about')->pluck('value');
        return $footer_setting[0];
    }
}
