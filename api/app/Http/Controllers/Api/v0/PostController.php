...
class PostController extends Controller
{
        ...
        if($post->save()) {
            ...
            if (!empty($pictures)) {
                foreach ($pictures as $list) {
                    $this->upload($list , $post->id , $existing_id, $request);
                }
            }
    ...
    public function save_draft(Request $request){
        ...
        if(!empty($request->draft_id)){
            $draft_post  = DraftPost::findOrFail($request->draft_id);
            $draft_post->fill($request->all());
            $draft_post->flag = 'autosaved';

            if($draft_post->save()) {
                return response([
                    'message' => 'Draft Post Updated Successfully',
                    'draft_data' => $draft_post
                ]);
            }
        }else{
            $draft_post  = new DraftPost();
            $draft_post->fill($request->all());
            $draft_post->flag = 'autosaved';
    ...
    private function save_history($post){
        $history  = new DraftPost();
        $history->post_id = $post->id;
        $history->title = $post->title;
        $history->slug = $post->slug;
        $history->short_description = $post->short_description;
        $history->description = $post->description;
        $history->video_link = $post->video_link;
