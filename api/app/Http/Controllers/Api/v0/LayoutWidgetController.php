...
class LayoutWidgetController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $data = $request->only('widget_type','taxonomy','category_taxonomy','data_limit','data','cat_data','widget_path', 'show_title', 'show_excerpt', 'title');
    ...
    public function layoutWidget(Request $request){
        ...
        if($request->category_taxonomy == 'category'){
            return Category::latest()->get();
        }elseif($request->category_taxonomy == 'post'){
            return Post::latest()->take(15)->get();
        }
    }
    ...
    public function filterData(Request $request){
        ...
        // Category Taxonomy 
        if ($request->category_taxonomy != ''){
            return  $post = Post::query()
                      ->whereLike(['title','short_description','description'], $request->category_taxonomy)->get();
        }
    }
}
