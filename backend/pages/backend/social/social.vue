    ...
    <v-container fluid>
      <v-row dense>
        <v-col class="col-12">
          <v-card>
            <!-- {{ socials ? socials[0] : ''}} -->
            <v-toolbar class="font-weight-bold" color="indigo" dark dense>
              <v-toolbar-title>Social link List</v-toolbar-title>
            </v-toolbar>
            
            <v-data-table :headers="headers" :items="socials" :search="search" id="printMe">
              <template v-slot:top>
                  <v-toolbar flat>
                    ...
                    <nuxt-link to="/social/addSocial">
                      <v-btn color="primary font-weight-bold" dark class="ml-3 mr-3" small>
                        Add Socials Link
                      </v-btn>
                    </nuxt-link>
                  </v-toolbar>
                ...
                <template v-slot:item.socials="{ item }">
                  <nuxt-link
                    :to="{ name: 'social-update-id', params: { id: item.id } }"
                  >
                    <v-icon small class="mr-2"> mdi-pencil </v-icon>
                  </nuxt-link>
                  <v-icon @click="deleteSocial(item.id)" color="#F44336">
                    mdi-trash-can
                  </v-icon>
                </template>
              ...
              <template v-slot:item.logo="{ item }">
                <v-img :lazy-src="item.logo" height="50" width="50" style="margin: 5px; border-radius: 10px" :src="item.logo" :alt="item.title"></v-img>
              </template>
              ...
              <template v-slot:item.isActive="{ item }">
                <v-btn x-small color="primary" dark class="mb-2" v-if="item.isActive == 1">
                  Active
                </v-btn>
...
export default {
  ...
  methods: {
    ...
    deleteConfirm() {
      try {
        this.$axios.delete("/socials/" + this.id)
        .then((res) => { 
          this.$axios
          .get("/socials")
          .then((res) => {
