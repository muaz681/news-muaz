<template>
  <div class="grid grid-cols-12" v-if="this.postsLoaded !== null">
    <div v-for="(post, i) in postsLoaded" :key="i" class="col-span-12 lg:col-span-6">
      <NuxtLink :to="{name: 'slug', params: {slug:decodeURIComponent(post.slug)}}" class="flex flex-row items-center md:flex-row md:max-w-xl">
        ...
        <div class="flex flex-col justify-between px-2 leading-normal">
          <p class="font-normal text-sm text-black line-clamp-3 font-sans" v-html="post.title">
          </p>
        </div>
      </NuxtLink>
    </div>
