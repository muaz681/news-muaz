<template>
  ...
  <!-- Content Section -->
    <div v-if="postsLoaded" class="grid grid-cols-12 lg:p-2 p-0 divide-y divide-zinc-300 max-h-96 overflow-y-scroll">
      <div v-for="(post, i) in postsLoaded" v-if="data_limit && data_limit >= i" :key="i" class="col-span-12 py-2">
        <NuxtLink :to="{name: 'slug', params: {slug:decodeURIComponent(post.slug)}}" class="flex flex-row items-center md:flex-row md:max-w-xl">
          ...
          <div class="flex flex-col justify-between px-2 leading-normal">
            ...
            <p v-if="widget_settings.show_excerpt == 1" class="font-normal text-xs text-black line-clamp-3 font-sans" v-html="post.short_description">
            </p>
<!--            <p v-if="widget_settings.show_excerpt == 1" class="font-normal text-sm text-black dark:text-gray-400 line-clamp-3 font-sans" v-html="post.short_description">-->
<!--            </p>-->
          </div>
        </NuxtLink>
    </div>
    </div>
    <div v-else class="grid grid-cols-12 lg:p-2 p-0 divide-y divide-zinc-300 max-h-96 overflow-y-scroll">
      <div v-for="(n,i) in 5"
